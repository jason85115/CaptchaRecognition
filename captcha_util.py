...
# Global Variable
CHARS = config.common_config['chars']  # 依據不同的驗證碼，要調整這個候選名單
...
def preprocess(imageName, imageOri):
    
    imageGray = cv2.cvtColor(imageOri, cv2.COLOR_BGR2GRAY)
    
    ret, imgBinary = cv2.threshold(imageGray, 145, 255, cv2.THRESH_BINARY_INV)
    
    contours, _ = cv2.findContours(imgBinary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    
    imageMask = np.zeros(imageOri.shape, np.uint8)
    cv2.drawContours(imageMask, contours, -1, (255,255,255), cv2.FILLED)
    
    imgResult = cv2.bitwise_and(imageOri, imageMask)
    
    cv2.imwrite('PreImages/' + imageName, imgResult)
    
    return imgResult

# 傳入oriImagePath需為完整路徑，且路徑為 / 非 \\
def rename2label(oriImagePath, predictLabel):
    assert '\\' not in str(oriImagePath), '路徑含有\，請使用/'
    oriImageName = oriImagePath.split('/')[-1]
    oriImageSubFileName = '.' + oriImagePath.split('/')[-1].split('.')[-1]
    oriImageDirectory = oriImagePath[0:oriImagePath.index(oriImageName, 0, len(oriImagePath))]
    newImagePath = oriImageDirectory + predictLabel + oriImageSubFileName
    try:
        os.rename(oriImagePath, newImagePath)
        print(oriImageName + ' --> ' + predictLabel + '.' + oriImageSubFileName)
    except Exception as ex:
        print(newImagePath +' '+ ex.args[1])
        
# 找檔名比最小長度還小的
def findSmallThanMinLength(imagesPath, minLength):
    images = glob(imagesPath + '*')
    for image in images:
        image = image.replace('\\', '/')
        imageName = image.split('/')[-1].split('.')[0]
        if len(imageName) < minLength:
            print (image.split('/')[-1])
